---
title: "Differential Analysis of 240814_Popzhelyazkova_Bremer_Neuropathologie_DNAmArray"
author:
  Joseph Kuo
...
Tag_this_analysis <- "DNAmArray_Differential_Analysis"

output_dir <- "../../output"
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)
load(file.path(output_dir, "DNAmArray_Processing.RData"))

# Define the threshold for significance (e.g., adjusted p-value < 0.05)
alpha <- 0.001
```

The steps done in this analysis are done according to [A cross-package Bioconductor workflow for analysing methylation array data](https://www.bioconductor.org/packages/release/workflows/vignettes/methylationArrayAnalysis/inst/doc/methylationArrayAnalysis.html).
...
hp <- pheatmap(heatmap_data,
         ...
         main = paste0("Heatmap of Top ", Num_top_CpGs, " CpGs in Each Comparison"),
         annotation_col = column_annotation,  # Add column annotation
         annotation_colors = annotation_colors # Apply custom colors
)


pdf(file.path(output_dir, paste0(Tag_this_analysis, "_top500_diff_CpGs.pdf")))
hp
dev.off()

postscript(paste0(Tag_this_analysis, "_top500_diff_CpGs.eps"),
           horizontal = FALSE, onefile = FALSE, paper = "special", width = 12, height = 10)
hp
dev.off()
...
## Heatmap of top 10 Sig. Probes in all comparisons
...
hp <- pheatmap(heatmap_data,
         ...
         clustering_distance_cols = "euclidean",
         clustering_method = "complete",
         show_rownames = TRUE,               # Optionally hide CpG names if too many
         show_colnames = TRUE,                # Show sample names
         border_color = NA,
         main = paste0("Heatmap of Top ", Num_top_CpGs, " CpGs in Each Comparison"),
         annotation_col = column_annotation,  # Add column annotation
...
pdf(file.path(output_dir, paste0(Tag_this_analysis, "_top10_diff_CpGs.pdf")), width = 12, height = 10)
hp
dev.off()

postscript(paste0(Tag_this_analysis, "_top10_diff_CpGs.eps"),
           horizontal = FALSE, onefile = FALSE, paper = "special", width = 12, height = 10)
hp
dev.off()
...
[Download PDF image](`r paste0(Tag_this_analysis, "_top500_diff_CpGs.pdf")`)
[Download EPS image](`r paste0(Tag_this_analysis, "_top500_diff_CpGs.eps")`)
...
# Subset relevant annotation columns from annEPIC
annEPIC_subset <- annEPIC[, c("Name", "chr", "pos", "strand", "UCSC_RefGene_Name", "UCSC_RefGene_Group")]
...
for (i in seq_along(comparison_names)) {
  ...
  merged_table <- merge(top_table, annEPIC_subset, by.x = "row.names", by.y = "Name", all.x = TRUE)
  colnames(merged_table)[1] <- "CpG_ID"
  ...
  writeData(wb, sheet = comparison_names[i], merged_table)
}
# Add a new sheet for heatmap_data
addWorksheet(wb, sheetName = "Heatmap of top CpGs")
# Write the heatmap_data to the worksheet
writeData(wb, sheet = "Heatmap of top CpGs", heatmap_data, colNames = TRUE, rowNames = TRUE)
# Save the workbook to a file
saveWorkbook(wb, file = file.path(output_dir, paste0(Tag_this_analysis, "_statistics.xlsx")), overwrite = TRUE)
```


```{r, echo=TRUE, results='asis', warning=FALSE, message=FALSE}
row_order <- hp$tree_row$order
col_order <- hp$tree_col$order
wb <- createWorkbook()
addWorksheet(wb, sheetName = "Heatmap of top CpGs")
writeData(wb, sheet = "Heatmap of top CpGs", heatmap_data[row_order,col_order], colNames = TRUE, rowNames = TRUE)
addWorksheet(wb, sheetName = "CpGs information")
writeData(wb, sheet = "CpGs information",
          annEPIC_subset[match(rownames(heatmap_data)[row_order], annEPIC_subset$Name),],
          colNames = TRUE, rowNames = TRUE)
saveWorkbook(wb, file = file.path(output_dir, paste0(Tag_this_analysis, "_heatmap.xlsx")), overwrite = TRUE)
```
...
save(comparison_names, fit2, sel_mValue, sel_bValue, file = file.path(output_dir, paste0(Tag_this_analysis, ".RData")))
```
